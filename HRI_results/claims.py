...
SET_CONSTRUCTION = [
    ...
    # ---- 2026-09-11: the de-selected sets under the UNIFIED predicates (DEMO_SETS_2026-09-11.md)
    dict(fact='Under the unified predicates the raw human set and the de-selected machine set '
              'are matched at every phase, within 0.06.',
         numbers='re-execution census, ladder=staged, {human dHfull_all n=74} v {machine '
                 'dDPfull_first n=72}: picked 0.88 v 0.89; placed_v2 0.57 v 0.61; contact_push '
                 '(release-gated) 0.14 v 0.19; pushed 0.38 v 0.39; nested_v2 0.19 v 0.22; '
                 'slide_success 0.18 (13) v 0.19 (14); nested_honest 0.23 v 0.25; tipped 0.18 v '
                 '0.17; sum reward 179 v 192 (cluster build of record 171 v 183). Route: slide '
                 '13 v 14, nested-by-drop 1 v 2.',
         source='HRI_results/DEMO_SETS_2026-09-11.md section 3; '
                'can_pos_recovery/videos_ladder_2026-09-11/census_{human,machine}.json; cluster '
                'build $W/demos_state_full/{dHfull_all,dDPfull_first}_rz (job 3537411, 64-core)',
         consequence='A within-learner human-versus-machine difference on the unified ladder is '
                     'not inherited from the demonstrations\' phase success. Sentence to avoid: '
                     '"the machine demonstrations are better (or worse) at the task" - at n=74 v '
                     '72 they are not distinguishable by phase. This supersedes the best-of-three '
                     'entry above for every comparison that uses dDPfull_first.'),
    dict(fact='The machine set\'s teacher was a Diffusion Policy trained on the PRUNED human '
              'full-task set, so the standing contrast is raw human demonstrations versus '
              'demonstrations from a pruned-trained imitator.',
         numbers='teacher dp_phase/dHfull_pruned_DP_s0: train_config.json dataset.root = '
                 'baselines/matched_w3/dHfull_pruned/lerobot; all 195 harvested tapes '
                 '(src_dDPfull) stamp that checkpoint as teacher_ckpt; dHfull_pruned = the same '
                 '74 recordings minus the 10 no-pick tapes (64), pre-pick idle collapsed only, '
                 'last-50 actions bit-identical to raw on 64/64.',
         source='verified 2026-09-11 from the cluster artefacts; PHASE_PLAN amendment (ab); '
                'HRI_results/DEMO_SETS_2026-09-11.md section 1',
         consequence='The arms differ in source AND in two pruning effects (pre-pick idle '
                     'collapse; the 10 no-pick tapes absent from the teacher\'s data). RLPD and '
                     'the world model were indifferent to pruning in pick scope; Diffusion Policy '
                     'was not (+0.19 to +0.27). State it as a property of the machine set; do not '
                     'write "pick-only" - the pruned set is full-task.'),
]
