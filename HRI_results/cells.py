...
COMPARISONS = [
    ...
    dict(id='pick_spots60_rlpd_sampled', expect_seeds=8,
         group='Pick (spots60, in-distribution)', learner='RLPD', stat='picked', action='sampled',
         human='dHv2raw (raw, 66)', machine='dDPv2 (raw-matched)',
         sel_h=dict(source='cluster:rlpd', arm='dHv2raw', cell='final_sampled_spots60/spots60',
                    extra_has='wave=rlpd_g99v2fullw3'),
         sel_m=dict(source='cluster:rlpd', arm='dDPv2', cell='final_sampled_spots60/spots60',
                    extra_has='wave=rlpd_g99v2fullw3'),
         note='The SAMPLED in-distribution cell, at full pinned seeds. Sampled is the statistic '
              'the project adopted for the online learners on 2026-09-07, so this is the '
              'in-distribution row to read for RLPD.'),
    ...
    dict(id='place_symmetry_r2d', expect_seeds=8, group='Place (matched 39)',
         learner='world model (r2dreamer)', stat='placed_v2', action='mode',
         human='dH_place (39)', machine='dDP_place_n39 (39)',
         sel_h=dict(source='cluster:wm', phase='place', arm='dH', cell='polEdDP_v2',
                    action_mode='mode', statistic='placed_v2', extra='setting=bnormclamp1ent5'),
         sel_m=dict(source='cluster:wm', phase='place', arm='dDP', cell='polEdDP_v2',
                    action_mode='mode', statistic='placed_v2',
                    extra='setting=bnormclamp1ent5_n39'),
         note='SYMMETRY CONTROL. Identical contrast to place_r2d, but scored on the bank of '
              'entry states generated by the MACHINE policy rather than the human one. It tests '
              'whether the place result is an artefact of whose policy produced the evaluation '
              'entries. It is not: the difference moves from +0.063 on the human-origin bank to '
              '+0.072 here, a BANK-OF-ORIGIN TERM OF 0.009 - an order of magnitude below the '
              'equivalence margin. Both banks give the same answer.'),
