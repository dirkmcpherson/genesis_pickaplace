...
| `SPARSE_LADDER_RESULTS_2026-09-15.md` | every human-v-machine cell under the sparse ladders (`nested_sparse` +1, `nested_sparse10` +10), state and pixel observations, three learners, rnd30 MODE at each seed's latest milestone, with Fisher + hierarchical-Bayesian columns and the disclosed pixel-vs-state design differences. Hand-maintained, not regenerated by `make_tables.py`. |
