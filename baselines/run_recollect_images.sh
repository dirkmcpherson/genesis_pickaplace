...
OUT=baselines/episodes_all_images
ARCHIVE=baselines/episodes_all_images_prev
...
for k in "${!CHUNKS[@]}"; do
  $PY baselines/collect_all_classified.py --outdir $OUT --images --uids ${CHUNKS[$k]} \
      > baselines/recollect_img_w$k.log 2>&1 &
done
wait
...
recs = {}
for p in sorted(glob.glob('baselines/episodes_all_images/_stage_*.json')):
    recs.update(json.loads(pl.Path(p).read_text()))
pl.Path('baselines/demo_manifest_images.json').write_text(json.dumps(
