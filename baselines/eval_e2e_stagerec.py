...
# ============================ LANE-1 INSTRUMENTATION (2026-09-10) ============================
# A DELIBERATE COPY of eval_e2e.py. The original is read at runtime by in-flight cluster eval
# stages and must not be mutated; and this copy must not be used to produce a cell of record.
# It adds ONE thing: a per-ENV-FRAME record of the poses / contacts / stage flags that
# baselines/stage_predicates.StageTracker consumes, written to <out>/frames/ep<k>.npz.
#
# Per ENV FRAME, not per decision: `FullTaskEnv.step` holds one decision for action_repeat=4
# calls of `_step_once`, and AT_REST_FRAMES=12 is 12 env frames = 3 decisions. Sampling once per
# decision would give 3 samples per window and could not evaluate the predicate at all. So the
# hook wraps `_step_once`, the env's own per-frame boundary.
#
# Everything recorded is a READ. The env already calls `bottle.get_contacts(goal)` on every frame
# and `goal.get_contacts(kinova)` on picked contact frames; `tool_pos()` reuses the eef pose reads
# the env makes for its own workspace guard. State reads do not perturb the solver (the #26 trace
# ablation; `end_of_episode()`'s docstring relies on the same fact). --check-determinism runs the
# episode twice, instrumented and not, and asserts the outcomes match.
...
NO_PROXY_TERM = os.environ.get('STAGEREC_NO_PROXY_TERMINATE', '') == '1'


def _install_frame_recorder(env):
    _orig = env._step_once

    def _hooked(action):
        out = _orig(action)
        _obs, _r, _term, _trunc, _info = out
        if NO_PROXY_TERM and _term and _info.get('nested'):
            # LADDER_UNIFY_BRIEF D2: under the new ladder the nested PROXY terminates nothing
            # (only slide_success and tipped do). The old env ends the episode on the first frame
            # the can touches the goal with the grip open -- i.e. while the can is still moving --
            # which is why `at_rest` cannot be satisfied before the horizon. Suppressing only that
            # branch, at the env's own per-frame boundary, measures `nested_v2` under D2's terminal
            # rule without editing full_env.py (Lane 2 owns it). The tip terminal is untouched and
            # reward is untouched (it does not affect a fixed policy's trajectory).
            _term = False
            out = (_obs, _r, False, _trunc, _info)
        w = env.genv.w
        bp = _np_(w['bottle'].get_pos()); bq = _np_(w['bottle'].get_quat())
        gp = _np_(w['goal'].get_pos()); gq = _np_(w['goal'].get_quat())
        tool = _np_(env.genv.tool_pos())
        c = _np_(w['bottle'].get_contacts(w['goal'])['position'])
        gg = _np_(w['goal'].get_contacts(w['kinova'])['position'])
        a = _np.asarray(action, dtype=_np.float64)
        grip = float((_np.clip(a[6], -1.0, 1.0) + 1.0) / 2.0) if env.action_mode == 'delta_joint' \
            else float(a[6])
        _FRAMES.append(dict(
            can_pos=_np.asarray(bp, _np.float64)[:3], can_quat=_np.asarray(bq, _np.float64)[:4],
            goal_pos=_np.asarray(gp, _np.float64)[:3], goal_quat=_np.asarray(gq, _np.float64)[:4],
            tool=_np.asarray(tool, _np.float64)[:3], wrist=_np.asarray(_np_(w['eef'].get_pos()), _np.float64)[:3],
            grip_cmd=grip,
...
_install_frame_recorder(env)
(OUT / 'frames').mkdir(parents=True, exist_ok=True)
print(f'[stagerec] per-env-frame recorder installed (action_repeat {REPEAT}; '
      f'{REPEAT} frames per decision) -> {OUT}/frames/', flush=True)
# ===========================================================================================
...
for k, ic in enumerate(ics):
    t0 = time.time()
    _CUR['ic'] = dict(ic)
    _FRAMES.clear()                       # [stagerec] one frame log per episode
    uid = ic.get('uid')
    ...
    # [stagerec] dump the per-env-frame log. Saved AFTER end_of_episode() only so the settled
    # columns can be stamped alongside; end_of_episode() never calls _step_once, so the frame
    # list is identical either way.
    _fp = OUT / 'frames' / f'ep{IC_OFFSET + k}.npz'
    _cols = {}
    for _key in _FRAMES[0].keys() if _FRAMES else []:
        _cols[_key] = _np.asarray([f[_key] for f in _FRAMES])
    _np.savez_compressed(
        _fp, n_frames=len(_FRAMES), ep=IC_OFFSET + k, uid=(-1 if uid is None else int(uid)),
        shelf_top_z=float(env.shelf_top_z), action_repeat=int(REPEAT),
        ic_can=_np.asarray(ic.get('can_pos', [_np.nan] * 3), _np.float64).reshape(-1)[:3],
