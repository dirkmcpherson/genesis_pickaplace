#!/usr/bin/env python
"""PIXEL end-to-end evaluator for {RLPD} `--obs pixels` checkpoints (lane PXR-1, 2026-09-13).

A COPY of baselines/eval_e2e.py (the live evaluator is read at run time by in-flight cluster jobs and is
left untouched) with exactly these differences, each marked `# PX:` below:
  * --kind sac only; the checkpoint sidecar must say obs == 'pixels' (a state checkpoint is refused);
  * the env is built with camera_rig=True and wrapped by rlpd_pixel.PixelObsWrapper -- the SAME observation
    function the trainer used (rig_obs() top RGB ++ wrist RGB once after reset and once after every decision,
    proprio = state[:8]); the policy sees {image, proprio} and never the can/goal pose;
  * IC injection, the unified predicates, the ladder/tip-guard sidecar rules, stage records, hardware stamps,
    the settle and the metrics.json layout are the original's, byte for byte; metrics.json additionally carries
    obs / image_shape / proprio_dim / n_renders.
The DP branch of the original is unreachable here (choices=('sac',)) and left in place so the diff stays small.
...
ap.add_argument('--kind', choices=('sac',), required=True)   # PX: pixel RLPD checkpoints only
...
if args.kind == 'sac':
    ...
    # PX: this evaluator feeds {image, proprio}; a state policy cannot consume it and a pixel policy cannot
    # consume the state evaluator's 17-dim vector -- the sidecar decides, never a default.
    assert side.get('obs') == 'pixels', f"eval_e2e_px.py needs a --obs pixels checkpoint; sidecar obs={side.get('obs')!r}"
...
env = FullTaskEnv(backend='cpu', max_steps=args.max_steps, scope='full', ladder=LADDER_NAME,
                  ...
                  camera_rig=True)   # PX: the two-camera rig rig_obs() renders from
...
# PX: the observation function of the trainer, verbatim (one module, one definition)
from rlpd_pixel import PixelObsWrapper, IMG_SHAPE, PROPRIO_DIM   # noqa: E402
pix = PixelObsWrapper(env)
assert side.get('image_shape') is None or tuple(side['image_shape']) == IMG_SHAPE, side.get('image_shape')
assert side.get('proprio_dim') is None or int(side['proprio_dim']) == PROPRIO_DIM, side.get('proprio_dim')
print(f'[eval-e2e] PX obs: {pix.observation_space} (rig_obs() top++wrist, proprio=state[:{PROPRIO_DIM}]; '
      f'sidecar image_aug={side.get("image_aug")} cnn_feature_dim={side.get("cnn_feature_dim")})', flush=True)
...
for k, ic in enumerate(ics):
    ...
    o = pix.observe(obs)                     # PX: render after the reset
    if k == 0:
        print(f'[eval-e2e] PX first frame: shape={o["image"].shape} dtype={o["image"].dtype} '
              f'min={int(o["image"].min())} max={int(o["image"].max())} mean={float(o["image"].mean()):.2f} '
              f'proprio={np.round(o["proprio"], 3).tolist()}', flush=True)
    policy_reset()
    frames = []
    if args.video:
    ...
    try:
        while not done:
            a = act(o)                        # PX: the dict observation
            if rec is not None:
            ...
            o = pix.observe(obs)              # PX: render after the decision
            ep_r += float(r); t += 1
            if args.video:
                frames.append(np.asarray(env.genv.w['cam'].render()[0])[:, :, ::-1])
...
summary = dict(checkpoint=str(ck), kind=args.kind, arm=args.arm, tag=args.tag, episodes=len(results), mode=args.mode, seed=args.seed,
               ...
               delta_cap=env.delta_cap, delta_leash=env.delta_leash, amendment='n+ladder-unify+px',
               # PX: what the policy observed
               obs='pixels', image_shape=list(IMG_SHAPE), proprio_dim=int(PROPRIO_DIM), n_renders=int(pix.n_renders),
               image_aug=side.get('image_aug'), cnn_feature_dim=side.get('cnn_feature_dim'),
