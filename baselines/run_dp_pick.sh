...
$LV/lerobot-train \
    ...
    --wandb.disable_artifact=true > baselines/train_pick.log 2>&1
...
bash baselines/dp_ckpt_curve.sh baselines/outputs/dp_pick $RUN >> baselines/ckpt_curve_chain.log 2>&1
...
$LV/python baselines/wandb_eval.py --kind dp \
    --checkpoint baselines/outputs/dp_pick/checkpoints/last/pretrained_model \
    --random 15 --seed 0 --group $RUN --name $RUN-eval \
    > baselines/eval_pick_wandb.log 2>&1
echo "[$(date)] pick EVAL exit $?" >> $CHAIN
tail -4 baselines/eval_pick_wandb.log >> $CHAIN
