...
**Consequence for these tables, and what this audit can and cannot decide.** Every cell of this sweep is *unpinned* — no `cpsc_*` launcher of mine ever set `TI_NUM_THREADS` or `OMP_NUM_THREADS` — so in all 128 comparisons the thread count WAS the physical core count. This audit therefore establishes the core-count rule and is silent on thread pinning by construction: it can neither support nor refute it. The thread-pinning evidence in the project record (`$W/rs2_threads.sbatch`, `TI_NUM_THREADS` ∈ {4, 8, 36} on one fixed end-to-end episode across nodes) is the eval-fixes agent's, not mine; where an earlier version of this doc said "if the pinning sweep succeeds, re-run pinned", that was deference to a sweep another agent was running, not a competing measurement of mine, and it is superseded here.

**Operational rule this audit supports: hold the physical core count fixed within any comparison.** It is checkable before submission (`sinfo -o '%n %c'`, or a job-side `grep -c ^processor /proc/cpuinfo` guard that fails fast on the wrong size) and needs no sweep. It is sufficient for every phase cell here: same-core reproduction was exact in 53/53 comparisons.
