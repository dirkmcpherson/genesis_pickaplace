...
def main():
    ...
    a = p.parse_args()
    if not 0 <= a.max_extension <= .01:
        raise ValueError('Registered extension bound is 0 to 10 mm')
    ...
    summary=dict(uid=uid,mode='eef_delta',variant=variant,variant_post_hook=True,can_pos=meta['can_pos'],
                 ...
                 extension_m=extension,insert_after=splice,repair_note=repair_note,sequence=score,
                 precision_ik=use_precision,hold_contact=a.hold_contact,
                 action_replay_verification=a.verify_actions,
                 provenance='real-derived bounded final-slide repair' if extension else 'real-derived EEF replay',
                 ...
                 code_sha256={str(path.relative_to(REPO)):hashlib.sha256(path.read_bytes()).hexdigest()
                              for path in [Path(__file__),REPO/'baselines/eef_delta_control.py',REPO/'can_pos_recovery/eef_task_sequence.py']})
