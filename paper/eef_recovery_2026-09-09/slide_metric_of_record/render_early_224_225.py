...
rows = []
for uid in [224, 225]:
    source = root.parent / 'early_yaw_pool' / str(uid) / 'collection' / f'{uid}_eef_delta.npz'
    rows.append(dict(uid=uid,source=str(source),source_sha256=hashlib.sha256(source.read_bytes()).hexdigest()))
variant = 'gc_kp4_riser3_shelf6_yaw17'
...
for record in rows:
    ...
    destination = root.parent / 'early_yaw_pool' / str(uid) / 'metric_visuals'
