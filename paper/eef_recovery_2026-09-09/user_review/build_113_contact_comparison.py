...
real=cv2.VideoCapture(str(raw/'output.mp4'));sim=cv2.VideoCapture(str(ROOT.parent/'early_contact_ts5/113/contact_probe_first30s.mp4'))
n=int(sim.get(cv2.CAP_PROP_FRAME_COUNT));fps=sim.get(cv2.CAP_PROP_FPS);assert n>0
out=ROOT/'113_real_sim_contact_probe.mp4';assert not out.exists()
...
pics=[];matches=[]
with np.load(ROOT.parent/'early_contact_ts5/113/collection/113_eef_delta.npz') as z:
 for j in range(n):
  ...
  labels=[(12,24,'113 | REAL camera 4'),(652,24,'SIM | contact-timeconstant EXPERIMENT, Day 1 yaw -9.7 deg'),(12,47,'Per-frame timestamps matched to bag receipt clock; acquisition delay uncalibrated'),
    ...
    (12,743,'Changed physics: earlier grasp retained; shelf-edge landing failure near 20.6s. Not an admitted recovery.')]
  ...
  if j in [0,140,200,226,230,n-1]:pics.append(cv2.resize(canvas,(768,456)))
w.release();real.release();sim.release();assert len(pics)==6
cv2.imwrite(str(ROOT/'113_contact_probe_comparison_phases.jpg'),np.vstack(pics))
(ROOT/'113_contact_probe_comparison.json').write_text(json.dumps(dict(matches=matches,max_abs_time_error_s=max(abs(r['time_error_s']) for r in matches),scope='First 30 seconds; saved measured-pose simulation, no independent successful replay claim; real timestamp matching is not acquisition calibration'),indent=2))
print('BUILT 113 CONTACT PROBE')
