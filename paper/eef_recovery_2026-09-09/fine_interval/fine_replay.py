...
def main():
    ...
    from fine_sequence import score_sequence, sustained_starts
    from fine_env import GenesisCanEnv
    from replay_harness import STATIC_BOTTLE_POSITION, BOX_SIZE, tilt_deg
    from sim_variant_hook import apply_pre, apply_post

    if a.max_extension or a.hold_contact:
        raise ValueError('Fine-interval experiment permits recorded-path reconstruction only')
    source = np.load(a.source)
    meta = json.loads(a.source.with_suffix('.json').read_text())
    if meta.get('decision_dt_s', meta.get('timing_reconstruction', {}).get('decision_dt_s')) != .01:
        raise ValueError('Expected an explicitly declared 10 ms source')
    ...
    used_code_sha256 = {str(path.relative_to(REPO)):hashlib.sha256(path.read_bytes()).hexdigest()
                       for path in [Path(__file__),REPO/'baselines/eef_delta_control.py',
                                    Path(__file__).with_name('fine_sequence.py'),Path(__file__).with_name('fine_env.py')]}
    ...
    summary=dict(uid=uid,mode='eef_delta',variant=variant,variant_post_hook=True,can_pos=meta['can_pos'],
                 ...
                 decision_dt_s=.01, physics_dt_s=.01, physics_steps_per_action=1,
    ...
    for key in ('grip_transform','terminal_hold_frames','suffix_offset_m',
                'timing_reconstruction','initial_pose_correction'):
        if key in meta:
