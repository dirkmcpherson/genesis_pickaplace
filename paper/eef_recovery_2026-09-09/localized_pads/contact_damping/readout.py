"""Retain damping attempts, exact controls and full-task outcomes separately."""
from pathlib import Path
import argparse, hashlib, json, sys
import numpy as np
HERE=Path(__file__).resolve().parent;R=HERE.parents[3];P=HERE.parent;POOL=P.parent
p=argparse.ArgumentParser();p.add_argument('--batch',type=Path,default=HERE/'corrected');a=p.parse_args();D=a.batch
...
for j in plan['jobs']:
 ...
 z=np.load(path);m=json.loads(path.with_suffix('.json').read_text());audit=json.loads((folder/'transmission_audit.json').read_text());stats=json.loads((folder/'transmission_stats.json').read_text());damp=json.loads((folder/'pad_damping_audit.json').read_text())
 ...
 assert damp['gain']==j['gain'] and max(damp['max_relative_error_static_normal_normal_velocity_tangent_velocity'])<1e-4
 assert audit['surface_pad_treatment']['contact_damping']['gain']==j['gain']
 if j['gain']==2:
  assert damp['applied_contact_rows']>0
  assert damp['min_applied_timeconst_s']>=damp['minimum_safe_timeconst_s']
 else:assert damp['applied_contact_rows']==0
 exact=None
 if j['reference']:
