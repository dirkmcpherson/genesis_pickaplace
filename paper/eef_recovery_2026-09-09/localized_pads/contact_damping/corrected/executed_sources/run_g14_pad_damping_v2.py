...
class DampedPads(Base):
    def __init__(self, *args, **kwargs):
        ...
        self.contact_data = self.contacts.contact_data
    ...
    def damp(self):
        for i in range(self.contacts.n_contacts[0]):
            ga, gb = self.contact_data.geom_a[i,0], self.contact_data.geom_b[i,0]
            cn, cp, penetration = self.contact_data.normal[i,0], self.contact_data.pos[i,0], self.contact_data.penetration[i,0]
            selected = 0
            for side in qd.static(range(2)):
                ...
                if self.enabled[g] and self.compliant[g]:
                    ...
                    if local[0] < -.008 and cosine > .5 and thickness > 0:
                        selected = 1
            if selected:
                old = self.contact_data.sol_params[i,0]
                new = old
                ...
                self.contact_data.sol_params[i,0] = new
