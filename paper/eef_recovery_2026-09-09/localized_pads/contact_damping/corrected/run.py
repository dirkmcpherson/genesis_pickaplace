...
R=Path(__file__).resolve().parents[5];D=Path(__file__).resolve().parent;P=D.parents[1]
assert not (D/'plan.json').exists()
(D/'logs').mkdir();(D/'executed_sources').mkdir()
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
files=['can_pos_recovery/run_g14_pad_damping_v2.py','can_pos_recovery/run_g14_hand.py',
...
def run(job):
 ...
 cmd=['/tmp/genesis-contact-1.4/bin/python','can_pos_recovery/run_g14_pad_damping_v2.py',job['source'],'--out',str(D/job['name']),'--preset',job['preset'],'--gain',str(job['gain'])]
