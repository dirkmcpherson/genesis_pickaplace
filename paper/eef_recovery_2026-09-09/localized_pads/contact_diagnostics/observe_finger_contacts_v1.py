...
class Contacts:
    def __init__(self, world):
        ...
        def observed():
            ...
            self.calls+=1
            if self.calls%world['scene'].sim.substeps==0:
                self.sample()
    ...
    def save(self):
        ...
        (out/'contact_dynamics_audit.json').write_text(json.dumps(dict(substep_calls=self.calls,
            sampling='One snapshot after constraint solve, before integration, at every eighth substep, including reset. Relative velocities precede current force integration.',
