"""Double only inner-pad normal reference damping; forward simulation diagnostic."""
        ...
        for order in range(self.contacts.n_contacts[0]):
            ...
            if selected:
                ...
                for direction in qd.static(range(3)):
                    ...
                    error=qd.abs(self.before[idx]-expected)/scale
                    if direction==0:
...
try:runpy.run_path(str(ROOT/'can_pos_recovery/run_g14_hand.py'),run_name='__main__')
    ...
    if instance is not None:
        report=dict(gain=a.gain,resolve_calls=instance.resolve_calls,
            ...
            qualification='Every assembled selected normal/tangent reference checked against installed imp_aref and current Jacobian velocity. Every unselected reference and all contact parameters/friction checked for exact nonmutation. Identity performs checks without writes.')
