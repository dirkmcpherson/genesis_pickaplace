...
import numpy as np
D=Path(__file__).resolve().parents[1];R=D.parents[3];V=D/'verification_retry_v3'
...
vplan=json.loads((V/'plan.json').read_text())
for f,h in vplan['source_code_sha256'].items():assert sha(V/'executed_sources'/Path(f).name)==h
for f,h in vplan['engine_source_sha256'].items():assert sha(f)==h
for j in vplan['jobs']:
 folder=V/j['name'];ex=json.loads((V/'logs'/f'{j["name"]}_execution.json').read_text());assert ex['returncode']==0
 z=np.load(folder/f'{j["uid"]}_eef_delta.npz');normal=json.loads((folder/'normal_damping_audit.json').read_text())
 assert normal['resolve_calls']==8+24*len(z['trajectory'])
 assert max(normal['max_input_scaled_normal_tangent_target_errors'])<1e-5
 assert normal['max_jacobian_entry_relative_error']<1e-6
 assert normal['counts_selected_normal_changed_normal_checked_unchanged_violations'][3]==0
identity={}
for name,uid in [('181_soft_g1',181),('176_soft_g2',176)]:
 ex=json.loads((V/'logs'/f'{name}_execution.json').read_text());assert ex['returncode']==0
 z=np.load(V/name/f'{uid}_eef_delta.npz');b=np.load(D/name/f'{uid}_eef_delta.npz');assert z.files==b.files
 identity[name]={k:bool(np.array_equal(z[k],b[k])) for k in z.files};assert all(identity[name].values())
(V/'identities.json').write_text(json.dumps(identity,indent=2))
...
for j in plan['jobs']:
 B=V if j['uid']==181 else D
 status=B/'logs'/f'{j["name"]}_execution.json';folder=B/j['name'];path=folder/f'{j["uid"]}_eef_delta.npz'
 ...
 if j['uid']==181:
  assert normal['audit_version']=='native_order_backward_error_v3'
  assert max(normal['max_input_scaled_normal_tangent_target_errors'])<1e-5
  assert normal['max_jacobian_entry_relative_error']<1e-6
 else:assert max(normal['max_normal_mapping_tangent_mapping_normal_target_relative_errors'])<1e-5
 ...
 row=dict(**j,status='complete',**score(path,folder/'metric_adapter'),frames=n,normal_damping_audit=normal,collision_policy=audit['collision_policy'],geometry=audit['geometry'])
...
report=dict(records=rows,original_references=references,fully_completed_uids=len(quartets),declared_uids=9,
 completed_full_replays=sum(r['status']=='complete' for r in rows),all_terminal=all(r['status']!='pending' for r in rows),
 totals_on_complete_quartets=totals(quartets),per_day_on_complete_quartets={day:totals([r for r in quartets if r['day']==day]) for day in ['Dec16','Dec17','Dec18']},paired_comparisons=comparisons,
 qualification='Aggregate comparisons use only complete four-condition UID groups, with denominator stated. Pending and implementation-failed runs retained separately. Same selected historical panel, not pristine unseen data, independent participants or population recovery. No admission without real-video/action verification.')
(V/'merged_summary.json').write_text(json.dumps(report,indent=2))
print('COMPLETED',report['completed_full_replays'],'/36; complete UID groups',len(quartets),'/9')
