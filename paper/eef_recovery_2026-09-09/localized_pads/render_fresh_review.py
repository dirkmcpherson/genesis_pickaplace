...
p.add_argument('--start-time',type=float,default=0.);p.add_argument('--stop-time',type=float,default=float('inf'));p.add_argument('--label',default='SOFT-PAD CANDIDATE')
p.add_argument('--reference',type=Path,default=REF)
a=p.parse_args();REF=a.reference;assert not a.out.exists()
...
frames=[i for i in range(5,len(data['trajectory']),6) if a.start_time<=(i+1)*.03<=a.stop_time]
assert frames
if a.stop_time>=len(data['trajectory'])*.03 and frames[-1]!=len(data['trajectory'])-1:frames.append(len(data['trajectory'])-1)
matches=[];stills=[]
try:
    for j,i in enumerate(frames):
        ...
        labels=[(12,26,f'Trial {uid} | {a.label} | recorded motion'),
        ...
        if any(abs((i+1)*.03-t)<.09 for t in [10,18,20.7,22.5]):stills.append(cv2.resize(canvas,(1008,588)))
...
a.out.with_suffix('.json').write_text(json.dumps(dict(trace=str(a.trace),trace_sha256=hashlib.sha256(a.trace.read_bytes()).hexdigest(),
    ...
    label=a.label,clip_start_s=(frames[0]+1)*.03,clip_end_s=(frames[-1]+1)*.03,render_code_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
