...
# Rotate about the initial can's two transverse axes (right multiplication),
# avoiding a nearly axial tool-y optimization coordinate.
reference_rotation = training[0]['pose'][:3,:3].T
def relative(x):
    return transform(x[:3], reference_rotation @ Rotation.from_rotvec([x[3],x[4],0]).as_matrix())
def project(x,c):
