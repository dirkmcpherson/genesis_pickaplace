...
parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--trace',type=Path,required=True);parser.add_argument('--out-prefix',type=Path,required=True);args=parser.parse_args()
anno=json.loads((R/'113_landmarks.json').read_text());path=args.trace;meta=json.loads(path.with_suffix('.json').read_text());source=np.load(meta['timing_reconstruction']['source']);z=np.load(path);tr=z['trajectory'];offset=float(source['t_frame'][0]);origin=json.loads((REF/'bag_origins.json').read_text())['113'];record=next(x for x in json.loads((REF/'manifest.json').read_text())['records'] if x['uid']==113);cam=next(x for x in record['cameras'] if x['camera']==4);times=(np.loadtxt(cam['timestamps'],dtype=np.int64)-origin)/1e9
...
args.out_prefix.with_suffix('.json').write_text(json.dumps(report,indent=2));print(json.dumps([dict(time=x['time_s'],nominal=x['nominal']) for x in rows],indent=2))
