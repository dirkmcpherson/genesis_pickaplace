...
p.add_argument('--steps',type=int,default=400);p.add_argument('--preset',type=Path,required=True)
a=p.parse_args();assert not a.out.exists();a.out.mkdir();assert a.n_envs>=0
B=max(1,a.n_envs);z=np.load(a.trace);meta=json.loads(a.trace.with_suffix('.json').read_text());preset=json.loads(a.preset.read_text());assert gs.__version__=='1.4.0'
N=min(a.steps,len(z['trajectory']));params=Parameters(**preset['transmission']);model=Transmission(params)
...
w['bottle'].set_pos(meta['can_pos']);w['bottle'].set_quat(meta.get('can_quat',[1,0,0,0]));w['bottle'].zero_all_dofs_velocity()
...
commands=torch.tensor(z['actions_joint'][:N],dtype=torch.float64,device=device)
fingers=torch.tensor(np.array([h.gripper_targets(float(g)) for g in z['source_grip'][:N]]),dtype=torch.float64,device=device)
def sync():
...
for i in range(N):
 target=fingers[i].expand(B,-1)
 robot.control_dofs_position(envshape(commands[i].expand(B,-1)),w['kdofs'][:6])
...
max_duplicate=float(np.max(abs(arr-arr[:,:1])))
np.savez_compressed(a.out/'trace.npz',state=arr,arm_commands=z['actions_joint'][:N],source_grip=z['source_grip'][:N])
report=dict(backend=a.backend,n_envs=a.n_envs,actual_envs=B,steps=N,build_reset_seconds=build_seconds,replay_seconds=end-start,warm_steps=max(0,N-50),warm_seconds=end-warm if warm else None,aggregate_decisions_per_second=B*(N-50)/(end-warm) if warm and N>50 else None,max_duplicate_state_difference=max_duplicate,max_torque=max_torque.cpu().tolist(),calls=calls,pad_calls=pad.calls,pad_counts=pad.counts.to_numpy().tolist(),engine=gs.__version__,device=str(device),source_trace=str(a.trace),source_sha256=hashlib.sha256(a.trace.read_bytes()).hexdigest(),preset_sha256=hashlib.sha256(a.preset.read_bytes()).hexdigest(),candidate_urdf_sha256=hashlib.sha256(urdf.read_bytes()).hexdigest(),geometry=[dict(link=l.name,geom=g.idx,vertices_sha256=hashlib.sha256(np.asarray(g._init_verts).tobytes()).hexdigest()) for l in robot.links for g in l.geoms],collision_pairs=solver.collider._valid_collision_pairs.tolist(),qualification='Benchmark only: identical episode copies, frozen archived joint/grip commands, normal damping1. GPU and batch equivalence and full-task export not yet established; no recovery claim or bank admission.')
