...
plan=json.loads((D/'repeat_plan.json').read_text())
cases=[{**c,'paths':[Path(p) for p in c['paths']]} for c in plan['cases']]
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
assert not (D/'repeat_execution.json').exists()
...
(D/'repeat_stack_probe.json').write_text(json.dumps(dict(returncode=probe.returncode,stdout=probe.stdout,stderr=probe.stderr),indent=2));assert probe.returncode==0,probe.stderr
rows=[]
for c in cases:
 ...
 (D/'repeat_execution.json').write_text(json.dumps(rows,indent=2));print('DONE',c['name'],r.returncode,flush=True)
