...
A=D/'supported_sources';A.mkdir(exist_ok=True)
for f in files:shutil.copy2(R/f,A/Path(f).name)
plan=dict(cases=[{**c,'paths':[str(p) for p in c['paths']]} for c in cases],steps=400,source_code_sha256={f:sha(R/f) for f in files},source_trace_sha256={str(p):sha(p) for p in [source233,source262]},installer_pid=pid,installer_proc_start_ticks=identity,
