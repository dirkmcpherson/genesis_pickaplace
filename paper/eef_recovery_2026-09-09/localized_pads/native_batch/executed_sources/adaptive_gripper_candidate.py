...
def make_urdf(path,distal_lower_limit=-.50):
    assert distal_lower_limit in (-.50,-1.03)
    ...
    for joint in root.findall('joint'):
        if joint.attrib['name'] in ('left_finger_tip_joint','right_finger_tip_joint'):
            ...
            if distal_lower_limit!=-.50:
                expected_axis='0 0 1' if joint.attrib['name'].startswith('right') else '0 0 -1'
                assert joint.find('axis').attrib['xyz']==expected_axis
                joint.find('limit').set('lower',str(distal_lower_limit))
    ...
    return dict(source=str(source),source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),
                ...
                distal_lower_limit_rad=distal_lower_limit,
                travel_reference='Kinova right-tip CAD range at c3280d8a6a6b5d96590d51538e3f265710a6c77d, mirrored by finger symmetry; raw left CAD limit signs differ. Not measured hardware travel.' if distal_lower_limit==-1.03 else 'Inherited ROS mimic-model range')
