...
plan=json.loads((D/'deterministic_plan.json').read_text())
cases=[{**c,'paths':[Path(p) for p in c['paths']]} for c in plan['cases']]
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
assert not (D/'deterministic_execution.json').exists()
...
(D/'deterministic_stack_probe.json').write_text(json.dumps(dict(returncode=probe.returncode,stdout=probe.stdout,stderr=probe.stderr),indent=2));assert probe.returncode==0,probe.stderr
rows=[]
for c in cases:
 for f,h in plan['source_code_sha256'].items():assert sha(R/f)==h
 for p in c['paths']:assert sha(p)==plan['source_trace_sha256'][str(p)]
 cmd=['/tmp/genesis-contact-1.4/bin/python','can_pos_recovery/bench_native_deterministic_g14.py','--trace',str(c['paths'][0]),'--preset',str(P/'proximal_compliance/presets/all_soft_control.json'),'--out',str(D/c['name']),'--backend','gpu','--n-envs',str(c['n_envs']),'--steps','400']
 ...
 (D/'deterministic_execution.json').write_text(json.dumps(rows,indent=2));print('DONE',c['name'],r.returncode,flush=True)
