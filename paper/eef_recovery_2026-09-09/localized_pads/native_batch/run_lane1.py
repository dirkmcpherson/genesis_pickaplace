...
plan=json.loads((D/'lane1_plan.json').read_text())
cases=[{**c,'paths':[Path(p) for p in c['paths']]} for c in plan['cases']]
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
assert not (D/'lane1_execution.json').exists()
...
(D/'lane1_stack_probe.json').write_text(json.dumps(dict(returncode=probe.returncode,stdout=probe.stdout,stderr=probe.stderr),indent=2));assert probe.returncode==0,probe.stderr
rows=[]
for c in cases:
 ...
 (D/'lane1_execution.json').write_text(json.dumps(rows,indent=2));print('DONE',c['name'],r.returncode,flush=True)
