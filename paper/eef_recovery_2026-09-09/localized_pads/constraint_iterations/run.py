"""Frozen iteration-cap refinement after observation-only baseline identity."""
...
assert all(json.loads((P / 'constraint_convergence/identity.json').read_text()).values())
(D / 'executed_sources').mkdir()
sha = lambda p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
files = ['can_pos_recovery/audit_g14_constraint_iterations.py',
...
plan = dict(source=str(source), preset=str(preset), reference=str(reference),
            ...
            purpose='Raise only CG iteration cap100 to1000, retaining float32, '
            '8 substeps and physical/source settings. No material or numerical change. '
            'Observation-only baseline already matches all14 arrays. Compare solver termination and full task; do not select a material by the outcome.')
(D / 'plan.json').write_text(json.dumps(plan, indent=2))
out = D / '233_cg1000'
cmd = ['/tmp/genesis-contact-1.4/bin/python', 'can_pos_recovery/audit_g14_constraint_iterations.py',
...
start = time.time()
with (D / '233_cg1000.log').open('w') as log:
