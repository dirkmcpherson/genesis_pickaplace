...
p.add_argument('--start-time',type=float,default=4.8);p.add_argument('--stop-time',type=float,default=8.4)
...
writer=cv2.VideoWriter(str(raw),cv2.VideoWriter_fourcc(*'mp4v'),1/.12,(1440,840));assert writer.isOpened()
...
frames=[i for i in range(len(data['trajectory'])) if a.start_time<=(i+1)*.03<=a.stop_time]
assert frames
matches=[];stills=[]
try:
    for j,i in enumerate(frames):
        ...
        labels=[(12,26,f'Trial {uid} | REJECTED SOFT HAND: APPROACH DIAGNOSTIC | 4x slow playback'),
        ...
        if any(abs((i+1)*.03-t)<.015 for t in [5.4,5.7,5.85,6.3,6.82,7.8]):stills.append(cv2.resize(canvas,(1008,588)))
...
a.out.with_suffix('.json').write_text(json.dumps(dict(trace=str(a.trace),trace_sha256=hashlib.sha256(a.trace.read_bytes()).hexdigest(),
    ...
    real_video=camera['video'],timestamp_sha256=camera['timestamp_sha256'],matches=matches,frames=len(frames),fps=1/.12,playback_speed=.25,
    ...
    scope='Saved simulation poses, no physics rerun or added settling. Four-times-slower diagnostic excerpt. Two real camera views matched by timestamps. Different viewpoints; not a registered overlay. This rejected hand fails pickup; not a new recovery gain.'),indent=2))
