"""Observe declared numerical reference solves with physical/source settings fixed."""
...
parser.add_argument('--solver', choices=['CG', 'Newton'], required=True)
...
def observed_world(*positional, **kwargs):
    kwargs['rigid_extra'] = dict(kwargs['rigid_extra'], iterations=args.iterations,
                               constraint_solver=getattr(gs.constraint_solver, args.solver),
                               tolerance=1e-8 if args.precision == '64' else 1e-5)
    ...
    assert str(qd_to_numpy(cs.grad).dtype) == 'float' + args.precision
    assert solver.rigid_config.solver_type == getattr(gs.constraint_solver, args.solver)
    assert int(qd_to_numpy(ri.iterations)) == args.iterations
    runtime.update(
