"""Frozen normal-only twofold pad damping: original calibration trio and conditional diagnostic."""
...
engine_files=['utils/geom.py','engine/solvers/rigid/constraint/solver.py','engine/solvers/rigid/rigid_solver.py']
for f in engine_files:shutil.copy2(engine/f,D/'executed_sources'/('engine_'+Path(f).name))
jobs=[]
for uid,placement in [(233,'original'),(113,'conditional'),(113,'original'),(184,'original')]:
...
plan=dict(jobs=jobs,workers=3,source_code_sha256={f:sha(R/f) for f in files},engine_source_sha256={f:sha(engine/f) for f in engine_files},
 hypothesis='Double only classified inner-pad normal reference damping before solve. Elastic term, all tangent references and contact parameters unchanged. Soft .03 and matching rigid .02 controls; no geometry or friction coefficient change.',
...
for wave in [jobs[:3],jobs[3:]]:
 ...
 if any(r['returncode'] for r in rows):
  (D/'implementation_gate_failed.json').write_text(json.dumps(dict(completed_jobs=len(rows),unstarted=[j['name'] for j in jobs[len(rows):]],reason='Implementation/verification failure. Do not treat unstarted jobs as recovery failures.'),indent=2))
  raise SystemExit(1)
