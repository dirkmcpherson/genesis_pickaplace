...
def sustained_starts(mask, length=3):
...
def score_sequence(data, shelf_rest_z=.2205, radius=.033):
    ...
    for touch in touch_starts:
        earlier_release = release_starts[release_starts + 3 <= touch]
        ...
        candidates = candidates[(candidates >= release + 3) & (candidates < touch)]
