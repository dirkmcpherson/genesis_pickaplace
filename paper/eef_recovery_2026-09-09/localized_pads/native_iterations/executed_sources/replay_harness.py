...
def reset_arm(kinova, kdofs):
    """Teleport the arm to HARDCODED_START AND re-issue it as the PD target.

    Port of GenesisCanEnv.reset's P2 FIX (2026-08-14) to the replay path (2026-09-03):
    set_dofs_position only moves qpos; the controllers keep the PREVIOUS episode's last
    control_dofs_position target (FORCE mode / zero torque in a fresh process), so the reset
    scene.step() ran under history-dependent control and a tape's stage depended on which
    tape ran before it in the same process (leak_probe.py: uid 242 -> nested solo, nested
    after 305, placed after 263 -- prox 0.068/0.069/0.081; with this re-issue all three are
    bit-identical, nested 0.0756). Every shard partition of a replay census was therefore a
    different experiment; use this in every replay-path reset."""
    kinova.set_dofs_position(np.array(HARDCODED_START), kdofs)
    kinova.zero_all_dofs_velocity()
    kinova.control_dofs_position(np.array(HARDCODED_START[:6]), dofs_idx_local=kdofs[:6])
    kinova.control_dofs_position(np.array(HARDCODED_START[-4:]),
...
def rollout(w, vel, gp, can_pos, goal_pos=None, max_cmds=None, stop_on_success=True,
    ...
    # reset (mirrors example.py setup(), plus velocity zeroing AND PD-target re-issue for
    # run-to-run isolation -- see reset_arm)
    reset_arm(kinova, kdofs)
