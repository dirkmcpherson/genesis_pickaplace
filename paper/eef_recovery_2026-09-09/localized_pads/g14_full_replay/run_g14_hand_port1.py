"""Isolated full-world Genesis1.4 replay using the declared adaptive hand."""
...
def candidate_world(*args,**kwargs):
    kwargs.update(urdf_file=str(urdf.resolve()),urdf_extra={'default_armature':.1},rigid_extra=opts)
...
state=None;pad=None;audit=None;pad_rows=[]
def candidate_post(env,variant):
    ...
    for row in json.loads(AUDIT.read_text())['records']:
        ...
        np.testing.assert_allclose(inertia,row['proposed_inertia'],atol=1e-11)
        np.testing.assert_allclose(mass,row['mass_kg'],rtol=1e-6)
        records.append(dict(link=row['link'],com=pos.tolist(),inertia=inertia.tolist(),mass_kg=mass))
    ...
    def observed(*args,**kwargs):
        ...
        assert state['substep_calls']-before==8 and pad.calls-pc==8
        pad_rows.append(pad.counts.to_numpy().copy());return result
    w['scene'].step=observed
    audit=dict(engine_version=gs.__version__,engine_options={k:str(v) for k,v in opts.items()},
...
try:runpy.run_path(str(REPO/'can_pos_recovery/repair_eef_slide.py'),run_name='__main__')
    ...
    if pad_rows:np.savez_compressed(a.out/'surface_pad_observations.npz',counts=np.asarray(pad_rows))
