...
plan=dict(uid=233,substeps=[8,16,32],source=str(source),source_sha256=sha(source),source_metadata_sha256=sha(source.with_suffix('.json')),preset=str(preset),preset_sha256=sha(preset),reference=str(reference),reference_sha256=sha(reference),source_code_sha256={f:sha(R/f) for f in files},workers=1,
 purpose='Numerical integration convergence diagnostic, motivated by extra-row and GPU dispatch sensitivity. Not a new material candidate or independent validation.',
 fixed='Same source duration/commands, initial geometry, masses, hand springs/damping, contact normal parameters, elliptic ratio10 and normal gain1. Scene dt.01, decision dt.03 fixed. Numerical substeps8/16/32 imply1.25/.625/.3125ms integration.',
...
for ss in plan['substeps']:
 ...
 cmd=['/tmp/genesis-contact-1.4/bin/python','can_pos_recovery/run_g14_hand_resolution.py',str(source),'--out',str(out),'--preset',str(preset),'--cone','elliptic','--impratio','10','--substeps',str(ss)]
