...
for uid in json.loads((root / 'plan.json').read_text())['uids']:
    ...
    execution = directory / 'result.json'
    if not execution.exists():
        continue
    result = json.loads(execution.read_text())
    sequence = result.get('score', {}).get('strict_contact_diagnostic', {})
...
report = dict(
    scope='Completed changed-contact early-day EEF trials failing the slide; experimental physics',
