...
| **Pick** (`spots60`, in-training-distribution) | **0.878 v 0.873, Δ +0.005, p 0.845, n 10 v 10** | **0.858 v 0.842, Δ +0.017, p 0.557** (sampled) / 0.867 v 0.865, p 1.000 (mode), n 8 v 8 | not run |
...
**The end-to-end null is a mid-flight snapshot.** Neither arm had converged at the budget — both were still improving in their final quarter — and the machine arm was climbing about twice as fast (picked +0.110 against +0.055, with 7 of 8 seeds still improving against 5 of 8). Combined with the budget confound below, "equivalent at 2e6" is a statement about a budget rather than about the learners.

**The end-to-end null carries a budget confound.** The two arms consume the fixed step budget differently — the machine arm ignites later and so gets less post-ignition training (60 % of the run remaining at the median against the human arm's 73 %, and 31 % for its worst seed). The bias runs *against* the machine arm, so equalising it could push the comparison outside the equivalence margin in the machine-favouring direction. This is a live threat to that row, not a conservatism argument.
