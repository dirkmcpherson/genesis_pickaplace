...
**CORRECTED 2026-09-09 — `contact_push` is a REAL MEASUREMENT, not an absence.** I first recorded the zero below as a missing field, diagnosed from reading code: the env never writes `contact_push` into `info`, so the grant looked unreachable. That inference was wrong. Probing a built environment shows `genv._contact_push` exists, is readable, and reports **False** — the attribute path is fine and the grant simply never fires. **These end-to-end policies genuinely never push the can from the far side.** It is consistent with everything else: 84–86 % of policy contact grants are carry-ins, and this arm reaches `nested` at 0.433 while `slide_success` sits at 0.267 — it gets there by carrying the can in, not by releasing and pushing. The statistic was doing its job. Original (incorrect) note follows for provenance.
