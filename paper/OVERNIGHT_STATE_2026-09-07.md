...
- **Hardware is partially confounded with ARM in the published 8 v 8, and the control to use is PHYSICAL CORE COUNT.** What is established, on Slurm's processor *counts*, which are reliable: **all 53 comparisons at the same core count were bit-identical**, across different nodes, different code versions and different CPU labels, while 19 of 75 comparisons at different core counts disagreed — every one of them with a 36-core machine on exactly one side.

  **What is NOT established, and where I over-corrected earlier tonight: that instruction set is irrelevant.** I first told you the divergence was AVX2 versus AVX-512, then withdrew that in favour of "CPU family is ruled out directly". Both statements leaned on Slurm's CPU-*family* labels, and those labels are wrong on this cluster — one node advertises Broadwell and is actually Cascade Lake. So the family question is **unresolved**, not settled either way, and it needs re-checking against the processor information the machines report about themselves.

  **This does not block anything**, which is why it is a footnote rather than a decision. Pinning core count is empirically sufficient on every comparison we have, it can be checked before submission rather than discovered afterwards, and it is available on any machine of the right size — so it is the control regardless of how the instruction-set question later resolves. The re-score hold has been released with that verdict.
