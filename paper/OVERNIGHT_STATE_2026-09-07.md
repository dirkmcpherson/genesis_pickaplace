...
- **Hardware is partially confounded with ARM in the published 8 v 8, and the responsible variable is now CORE COUNT, not instruction set.** I told you earlier tonight that AVX2 versus AVX-512 was the cause; that was wrong and is withdrawn. The dissociation is clean: a 40-core Broadwell and a 64-core Sapphire Rapids **agree** bit-for-bit across the instruction-set boundary, while a 36-core Broadwell **disagrees** with the 40-core Broadwell on the same instruction set. All 53 same-core-count comparisons are bit-identical, and all 19 differing pairs have a 36-core machine on exactly one side. The confound itself stands — hardware still tracks arm in the published cells — but the fix changes shape, and in our favour: pinning a fixed core and thread count can be done on **any** node, where pinning to two verified AVX2 nodes could not. **On the fix, there is a conflict in our own record that needs resolving before any re-score.** The 03:30 entry below states flatly that thread pinning does NOT work, with direct evidence: a 40-core node returns the same wrong outcome at thread counts 4, 8 and 36, a 64-core node likewise, while the 36-core node returns its own outcome at 4 and 36 — so the divergence is arithmetic-level, not thread-level. If that stands, the rule is simply **hold core count fixed within any comparison**, which is checkable in advance and needs no sweep. But the agent that produced that evidence later described a thread-pinning sweep as still pending, so either it means a larger sweep than the probe it already ran, or the phrasing is stale. **Resolve which before releasing the re-score hold** — the launcher is deliberately blocked until a verdict is set, and the two possible verdicts lead to different designs.
