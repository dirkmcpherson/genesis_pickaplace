...
| dv3 (DreamerV3-torch), state input, **sampled** | **0.700** (42/60) | **0.633** (38/60) | +0.067 | p 0.333 = the test's floor at n = 2 v 2 (no power) | **working configuration found 2026-09-07** (§7 item 9); compare with the r2dreamer **sampled** row above, not the mode row |
...
9. dv3 — **superseded 2026-09-07: a working configuration now exists.** The earlier statement ("every lever tried on the reach proxy is bimodal across seeds — baseline 1 & 0 of 15; fp32 15 & 0; end-effector actions 11 & 3; both 4 & 15 — no pick run exists, so it is excluded as 'no working configuration'") described the PIXEL reach proxy. Given the same state input and the same return clamp that fixed r2dreamer, dv3 learns the pick: gate G2 passed on both human seeds at 0.700 on the out-of-distribution set (registered threshold 0.5), with in-distribution 15/15, 14/15, 15/15, 15/15 across the four runs. **All dv3 cells are SAMPLED-action cells** — its evaluator defaults to `actor.sample()` and the runs record `eval_action_mode = sample`; the sidecar label reading 'deterministic' is a known mislabel. They must therefore be read against r2dreamer's sampled row (0.613 human v 0.629 machine), not its mode row. Partial comparison at n = 2 v 2: human 0.700 (42/60) vs machine 0.633 (38/60), exact p 0.333, which is the permutation test's own floor at that n — the partial null has **zero** power and should not be quoted as evidence of anything. The 4-v-4 completion finishes ≈ 14:00–15:00 on 2026-09-08; at n = 4 v 4 the smallest attainable p is 0.029, so significance would require the single most extreme arrangement. A mode-action sweep over all eight pick runs is queued so dv3 reports both action modes like r2dreamer. **Why this matters beyond dv3:** the clamp diagnosis reproduces on an independent port, so it is a property of the reward scale and not of one implementation, and the world-model arm is no longer a single-port result. See `DV3_DEBUG_2026-09-05.md` §8.
