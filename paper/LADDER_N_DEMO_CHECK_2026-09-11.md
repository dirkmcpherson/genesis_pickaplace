...
| offline `staged` vs **Lane 5's independent direct re-execution**, all 74 human tapes | **74/74 tapes identical**, Σ 179.0 = 179.0, every rung count and end reason identical |
| offline `staged` vs the same census, all 72 machine tapes | **72/72 tapes identical**, Σ 192.0 = 192.0 |
| offline `sparse` vs the same census, all 74 human | Σ 14.0 = 14.0 |

The census rows are the strongest of these: Lane 5 built their census by direct re-execution
with their own script, and the offline path reproduces it **tape for tape on all 146**.

One further check, after a late fix to the reward loop (a rung whose requirement first fires on
the same frame used to be deferred to the next one — harmless unless the rung is the terminal,
which `home` is): re-scoring the whole human set before and after the fix changes **0 of 74**
tapes. The fix is correct and the numbers here are unaffected either way.
...
Full grid: `paper/LADDER_N_CALIBRATION_GRID_2026-09-11.txt` (160 cells, both arms, latched and
unlatched), regenerated by `baselines/diagnostics/ladder_n_slide_calibration.py`; the
per-clause audit is `paper/LADDER_N_CLAUSE_AUDIT_2026-09-11.txt`
(`baselines/diagnostics/ladder_n_clause_audit.py`). Cells below are
...
makes that visible as a reward difference rather than an annotation. Neither arm is
disadvantaged by the definition: both lose only their drop-route arrivals, and both keep every
slide they have.

For contrast, turning `far_release` ON gives human 13 / machine 12 — the same human number and
two fewer machine slides, which is the asymmetry §4 argues against.

### The ramp span is the one constant the coverage test does not settle

Under `nested_ramp` at the registered 0.10 m span, the 13 human sim-slides earn **7.2 to 8.3 of
9, mean 7.80 (80–92 %)**; the 14 machine ones earn 7.3 to 8.4, mean 7.85. None reaches the
maximum, and the reason is only the span: their `slide_gain_m` runs 1.1–6.4 cm (human) and
1.3–9.3 cm (machine), against a span of 10 cm.

| span | human mean of 9 | human tapes at the maximum |
|---|---|---|
| 0.10 m (registered) | 7.80 | 0 of 13 |
| **0.05 m** | **8.39** | **4 of 13** |

If "the human set should earn near the maximum" is to be true of the ramp as well as of the
rungs, the span should be **0.05 m**. Left at 0.10 m here and recorded as an open decision
rather than changed unilaterally: unlike the reach, the span is not falsified by the coverage
test — it changes how much a slide is worth, not whether it is seen.

`slide_gain_m` distribution: human p50 0.0 cm (most tapes never slide), p90 4.7 cm, max
10.1 cm; machine p50 0.0, p90 5.4 cm, max 9.1 cm.
...
**The policy-side check could not be run, and that is itself the finding.** Lane 9's 315
rollouts are stored as per-episode outcomes, ICs and videos (`slide_smoke/*/metrics.json`) —
**no per-frame trajectories** — so their 23 `slide_success` positives cannot be re-scored under
the calibrated predicate without re-running the rollouts. Everything needed for the re-run
exists (the ICs are in each `per_episode` row and the checkpoints are on disk), so it is
possible; it was not attempted here.

This is the same problem the stage records solve for demonstrations, and it has the same fix.
**Recommendation: give `eval_e2e.py` (and the r2dreamer evaluator) the `--records-out` stage
recording that `relabel_reward.py` now has.** An evaluation that writes per-frame stage records
costs ~50 KB per episode and makes every future predicate question a 2-second offline re-score
instead of a re-simulation — including this one, and including the golden-set validation
already proposed. Without it, every predicate revision re-runs the policies.
...
`nested_ramp` sets carry Σ 219.2 and Σ 225.7, with the sim-slide tapes at mean 7.80 and 7.85
of 9. Compare the pilot's `sparse` (`nested_v2`): 14 and 16 — the difference is the drop route.

uids paying `home` — human: 232, 233, 237, 247, 251, 256, 259, 273, 275, 302, 304, 316, 317;
machine: 233, 242, 243, 251, 255, 259, 262, 263, 265, 281, 302, 305, 311, 321.
...
3. The ramp span of 0.10 m is never saturated by a human demonstration: the 13 human slides
   earn mean **7.80 of 9** under `nested_ramp` (80–92 %), and 0 of 13 reach the maximum. At a
   span of 0.05 m the same tapes average 8.39 and 4 reach it. **Open decision**, deliberately
   not taken here.
