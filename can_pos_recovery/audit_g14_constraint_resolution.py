...
parser.add_argument('--substeps', type=int, choices=[16], required=True)
...
sys.argv = ['run_g14_hand_resolution_v2.py', str(args.source), '--out', str(args.out),
            '--preset', str(args.preset), '--cone', 'elliptic', '--impratio', '10', '--substeps', str(args.substeps)]
try:
    runpy.run_path(str(ROOT / 'can_pos_recovery/run_g14_hand_resolution_v2.py'), run_name='__main__')
finally:
    if args.out.exists():
