...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--refresh-before-read',action='store_true');a=p.parse_args();assert not a.out.exists();a.out.mkdir(parents=True)
...
def observed(f):
 ...
 subrows.append(np.r_[len(subrows)*.00125,cached,physical,q,v,torque]);return step(f)
