...
if [ "$DEMO_FORMAT" = native ]; then
  case "$ARM" in
    dH|dDP|dR2D|dDPfails|dR2DDPfails|dHHfails|dR2DR2Dfails|dHsucc_dup|dDPsucc_dup) ARM_DEMO=$DEMO_ROOT/$ARM; PAT='^[0-9]{6}\.npz$' ;;   # dHHfails/dR2DR2Dfails: same-source fails arms (make_samesource_fails_arm.py)
