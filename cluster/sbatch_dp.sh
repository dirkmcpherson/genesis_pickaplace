...
if [ "$DEMO_FORMAT" = native ]; then
  case "$ARM" in
    dH|dDP|dR2D|dHunpruned|dHallpruned_1e3|dHallpruned_1e2|dDPallpruned_1e3|dDPallpruned_1e2|dH_A|dH_B|dDP_A|dDP_B|dR2D_A|dR2D_B|dHv2|dDPv2|dHv2raw|dDPv2p)   # dDPv2p: PREREG A31 DP machine arm (dDPv2 harvest per-IC matched vs the PRUNED dHv2 base)
      RAW=$DEMO_ROOT/$ARM
