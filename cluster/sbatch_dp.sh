...
if [ "$DEMO_FORMAT" = native ]; then
  case "$ARM" in
    dH|dDP|dR2D|dHunpruned|dHallpruned_1e3|dHallpruned_1e2|dDPallpruned_1e3|dDPallpruned_1e2|dH_A|dH_B|dDP_A|dDP_B|dR2D_A|dR2D_B|dHv2|dDPv2|dHv2raw|dDPv2p|dHfull_pruned)   # dHfull_pruned: PHASE PLAN 2026-09-04 full-task teacher set   # dDPv2p: PREREG A31 DP machine arm (dDPv2 harvest per-IC matched vs the PRUNED dHv2 base)
      RAW=$DEMO_ROOT/$ARM
...
# (shared, re-runnable stage: cluster/dp_select_confirm.sh -- same logic, same DP-HEADLINE line)
set +e   # evals must never fail an already-trained job
OUT="$OUT" ARM="$ARM" SEED="$SEED" RUN_NAME="$RUN_NAME" PROJ="$PROJ" ACTION_REPEAT="$ACTION_REPEAT" \
  EVAL_HORIZON="$EVAL_HORIZON" IC_FILE="$IC_FILE" bash cluster/dp_select_confirm.sh
echo "JOB DONE $(date)"
