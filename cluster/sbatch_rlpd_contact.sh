#!/bin/bash
# RLPD CONTACT/SLIDE-phase launcher (PHASE_PLAN_2026-09-04 amendment (m), 2026-09-07): the RLPD recipe of record
# (cluster/sbatch_rlpd.sh: UTD 10, E10/Z2, LN critics, gamma 0.99, 50/50 demo batches, delta_joint cap 0.025 /
# leash 5x, delta_ref target, action_repeat 4) trained in FullTaskEnv(scope='contact', phase_sparse) from the human
# CONTACT bank (banked placed_v2 states: released, on the shelf), demo half = the r2dreamer-native contact SEGMENTS
# (the rows the WM contact arm trained on), then the 4 evaluation cells (holdE_contact/polE_contact x sample/mode) of
# cluster/place_eval_cells.sh PHASE=contact on the LAST checkpoint. Reward = the env's bare `contact` grant (amendment
# (l)(d): rewards are unchanged); the STATISTIC of record is slide_success, computed at eval by the landed (l') code.
# Both arms are BELOW the registered 20-demo floor (11 v 11) and the pair is reported as sub-floor, as the WM arm is.
#
# Submit (from the code checkout root; W = the WM-fix dir with phase_banks/ and demos_state/):
#   for S in $(seq 0 7); do ARM=dH  SEED=$S sbatch -J sl_rlpd_dH_s$S  cluster/sbatch_rlpd_contact.sh; done
#   for S in $(seq 0 7); do ARM=dDP SEED=$S sbatch -J sl_rlpd_dDP_s$S cluster/sbatch_rlpd_contact.sh; done
# Env vars:
#   ARM       dH | dDP  (dH -> $W/demos_state/dH_contact, 11 human segments; dDP -> $W/demos_state/dDP_contact_n11, matched 11)
#   SEED      required
#   STEPS     250000 decisions (= 1e6 sim steps at repeat 4; amendment (h))
#   BANK      $W/phase_banks/human_contact.json (the HUMAN placed_v2 bank, 39 entries, for BOTH arms; amendment (m))
#   HOLDE / POLE   evaluation banks (defaults holdE_contact.json, polE_contact_physgrip.json = the eval-fixes agent's
#             REBUILT physical-grip bank of record, bank_version physgrip_2026-09-07, sha 219cb48e70d3bf9c)
#   OUT_ROOT  baselines/rl/checkpoints/contact  -> $OUT_ROOT/sl_rlpd_${ARM}_s${SEED}
...
#             periodic snapshot zips are OFF (--ckpt-every 0). 3 x 12 MB per run.
#   WAVE      contact SIM_VARIANT gc_kp4_riser3_shelf6   GAMMA 0.99   DRYRUN=1 prints the plan
...
#SBATCH --output=sl_rlpd_%j.out
#SBATCH --error=sl_rlpd_%j.out
...
case "$ARM" in
  dH)  DEMO=${DEMO:-$W/demos_state/dH_contact} ;;
  dDP) DEMO=${DEMO:-$W/demos_state/dDP_contact_n11} ;;
  *) echo "FATAL: ARM must be dH | dDP (got $ARM)"; exit 1 ;;
esac
BANK=${BANK:-$W/phase_banks/human_contact.json}
HOLDE=${HOLDE:-$W/phase_banks/holdE_contact.json}; POLE=${POLE:-$W/phase_banks/polE_contact_physgrip.json}
OUT_ROOT=${OUT_ROOT:-baselines/rl/checkpoints/contact}
OUT=$OUT_ROOT/sl_rlpd_${ARM}_s${SEED}
RUN_NAME="sl_rlpd_${ARM}_s${SEED}"
...
assert m['sim_variant'] == sv and m.get('phase') == 'contact' and m.get('with_state') is True and int(m['action_repeat']) == 4, m
assert abs(float(m['terminal_reward']) - 1.0) < 1e-9 and abs(float(m['delta_cap']) - 0.025) < 1e-9, m
fs = sorted(glob.glob(os.path.join(d, '*.npz'))); assert len(fs) == int(m['n_written']) == 11, (len(fs), m.get('n_written'))
if arm == 'dDP': assert m.get('matched_n_of') == 25 and m.get('matched_n_seed') == 0 and m.get('one_per_ic') is True, m
h = hashlib.sha256()
for f in fs:
    h.update(os.path.basename(f).encode()); h.update(open(f, 'rb').read())
sha = h.hexdigest()
print(f'DEMO-SHA {arm} contact-segment n={len(fs)} sha={sha[:16]} total_reward={m["total_reward"]} decisions_p50={m["decisions_median"]}', file=sys.stderr)
print(sha[:16])
PY
) || exit 1
...
REG_KNOBS=(steps="$STEPS" budget_unit=decisions scope=contact action_mode=delta_joint delta_ref=target action_repeat="$ACTION_REPEAT"
...
echo "== RLPD-CONTACT $RUN_NAME start $(date) node=$NODE_CLASS host=$(hostname) demo=$DEMO sha=$DEMO_SHA bank=$BANK restart=${SLURM_RESTART_COUNT:-0}"
if [ "${SLURM_RESTART_COUNT:-0}" -eq 0 ]; then
  python3 cluster/run_registry.py check --script sbatch_rlpd_contact.sh --arm "$ARM" --seed "$SEED" --demo-dir "$DEMO" --registry cluster/RUN_REGISTRY.jsonl "${REG_KNOBS[@]}"
  python3 cluster/run_registry.py register --script sbatch_rlpd_contact.sh --arm "$ARM" --seed "$SEED" --demo-dir "$DEMO" --registry cluster/RUN_REGISTRY.jsonl "${REG_KNOBS[@]}" --stage start
else
...
assert sc['scope'] == 'contact' and int(sc['steps']) == steps and sc.get('phase_sparse') is True and abs(float(sc['delta_cap']) - 0.025) < 1e-9, sc
...
print(f'TRAIN-OK {out}: budget {steps} decisions reached (ckpt_100 at {ck["ckpt_step"]}), sidecar scope=contact phase_sparse cap={sc["delta_cap"]} leash={sc["delta_leash"]}')
PY
set +e   # evals never fail an already-trained job
PHASE=contact KIND=sac CKPT="$FINAL_CK" OUT="$OUT" ARM="$ARM" SEED="$SEED" HOLDE="$HOLDE" POLE="$POLE" SIM_VARIANT="$SIM_VARIANT" PAR=4 bash cluster/place_eval_cells.sh 2>&1 | tee "$OUT/contact_eval.log"
echo "JOB DONE $(date)"
