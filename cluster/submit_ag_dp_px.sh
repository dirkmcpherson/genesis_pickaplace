#!/usr/bin/env bash
# PHASE_PLAN amendment (ag): {Diffusion Policy} on PIXEL observations, human v machine, 4 v 4.
# Registered 2026-09-14 BEFORE any build. Mirrors submit_aa_rev5_sparse_packs.sh's conventions.
#
#   GP=$LAB/gp_ag bash cluster/submit_ag_dp_px.sh          # submits 8 jobs
#   GP=$LAB/gp_ag DRYRUN=1 bash cluster/submit_ag_dp_px.sh # prints what each job would do, submits nothing
#
# GP must be a FRESH clone (gp_ag). It is refused if it names any of the pinned trees that in-flight jobs
# import at run time.
set -euo pipefail
LAB=${LAB:-/cluster/tufts/shortlab/jstale02}; W=${W:-$LAB/wm_fix_2026-09-03}
: "${GP:?set GP to the fresh (ag) clone (gp_ag); never a pinned tree}"
case "$GP" in
...
# ---- the datasets must exist and must still pass the action gate (cluster/ag_build_pixel_sets.sh) ----
for SET in dHfull_all dDPfull_first; do
  [ -d "$DEMO_ROOT/$SET/lerobot_px" ] || { echo "FATAL: $DEMO_ROOT/$SET/lerobot_px missing -- run cluster/ag_build_pixel_sets.sh first"; exit 1; }
  python3 "$GP/baselines/verify_px_lerobot.py" --px "$DEMO_ROOT/$SET/lerobot_px" \
...
Q=(-p gpu --qos=normal)
one () {  # arm seed
  local arm=$1 s=$2 name="ag_dp_px_${1}_s${2}"
