...
set -e; source /cluster/tufts/shortlab/jstale02/genesis_pickaplace/cluster/a31_chain/common.sh; STAGE=launch; cd "$GPR" || exit 1; ALLT=()
...
clog "=== A31 chain RESUME-FROM-SELECT (teachers done) (git $(git rev-parse --short HEAD)) ==="
declare -A MERGE
for W in old w3; do world_cfg $W
  T=()
  SEL=$(sub -p batch -c 1 --mem 2g -t 0:20:00 -J a31sel_$W -o $CHAIN_LOGDIR/select_${W}_%j.out cluster/a31_chain/select.sh $W)
  clog "select $W: job $SEL (no dependency; teachers already complete)"
...
clog "=== chain resumed from select ==="
