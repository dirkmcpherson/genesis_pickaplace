...
#   DV3_DIR       (default: ../dreamerv3-torch)
...
echo "== paper smoke harvest start $(date)  teacher=$TEACHER_CKPT  target=${SMOKE_KEPT:-20}"
python baselines/harvest_ai_demos.py --teacher-type dp \
  ...
  --outdir paper_smoke/m1_harvest

echo "== harvest manifest:"
python - <<'PY'
import json, sys
m = json.load(open('paper_smoke/m1_harvest/manifest.json'))
print(json.dumps(m, indent=2))
kept = m.get('kept', 0)
rej = m.get('rejected_by_verify', m.get('n_reject_verify', 0))
att = kept + rej
if kept < 12:
    print(f'FATAL: only {kept} kept -- teacher or harvest path broken'); sys.exit(1)
...
echo "== converting to pick-scope dreamer demos -> $DV3_DIR/demonstrations/genesis_m1_smoke"
python baselines/rl/to_dreamer_demos.py --scope pick \
  --src paper_smoke/m1_harvest \
  --dst "$DV3_DIR/demonstrations/genesis_m1_smoke"
python - "$DV3_DIR/demonstrations/genesis_m1_smoke" <<'PY'
import glob, sys
import numpy as np
...
if rew < 12:
    print('FATAL: <12 rewarded demos -- env pick grant vs relabel proxy mismatch?')
    sys.exit(1)
PY
