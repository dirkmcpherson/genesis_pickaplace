...
#   <RUN>/sweep/HEADLINE_sampled.txt   one SAMPLED-HEADLINE line
#
...
PY
SW=$RUN/sweep
if [ "${REDO:-0}" = "1" ]; then rm -rf "$SW/final_sampled" "$SW/final_det15"; fi
...
python3 - "$SW/final_sampled/sweep.json" "$SW/final_det15/sweep.json" "$ARM" "$SEED" "$CKPT_TAG" "$NODE_CLASS" "$IC_FILE" <<'PY'
import json, sys
smp, det, arm, seed, tag, node, icf = sys.argv[1:8]
def rd(p, s):
    try:
...
line = (f'SAMPLED-HEADLINE arm={arm} seed={seed} ckpt={tag} ic={icf} smp_hold={rd(smp,"hold")} smp_rnd={rd(smp,"rnd")} '
        f'det_hold15={rd(det,"hold")} node={node}')
print(line)
